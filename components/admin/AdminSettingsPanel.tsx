...
export default function AdminSettingsPanel({
  ...
  const [telegramUsername, setTelegramUsername] = useState(settings.telegram_username)
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const cleanedTelegramUsername = telegramUsername
      .trim()
      .replace(/^https?:\/\/(www\.)?t\.me\//i, '')
      .replace(/^@/, '')
      .split(/[/?]/)[0]

    if (!/^[a-zA-Z0-9_]{5,32}$/.test(cleanedTelegramUsername)) {
      setError('Telegram username must be 5-32 characters (letters, numbers, underscores only).')
      return
    }
    ...
    try {
      const response = await fetch('/api/admin/settings', {
        ...
        body: JSON.stringify({
          ...
          telegram_username: cleanedTelegramUsername,
        }),
      })
        ...
        <section>
          ...
          <div className="rounded-lg border-[2.5px] border-ink bg-paper p-4">
            <p className="mb-3 text-sm font-medium text-ink-soft">
              When an artist taps &quot;Buy Now&quot; on the Pricing page, Telegram opens a
              chat with this account and a pre-filled message naming the plan.
              Complete the payment in the chat, then unlock their access from
              the Artists tab.
            </p>
            <label
            ...
            <div className="flex items-center gap-2">
              <span className="flex h-11 items-center rounded-lg border-[3px] border-ink bg-white px-3 font-mono text-sm font-bold text-ink-faint">
