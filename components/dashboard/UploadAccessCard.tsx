...
export function UploadAccessLocked() {
  ...
  const [telegramUsername, setTelegramUsername] = useState('Dex_Error_404')
  ...
  useEffect(() => {
    ...
    fetch('/api/settings')
      ...
      .then((settings) => {
        if (active && settings?.telegram_username) setTelegramUsername(settings.telegram_username)
      })
      .catch(() => {})
    return () => { active = false }
  }, [])
